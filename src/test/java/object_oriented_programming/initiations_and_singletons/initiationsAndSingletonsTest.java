...
public class InitiationsAndSingletonsTest {
    ...
    private void checkVehicleStateIfDayOfWeekIs(int dayOfWeek) {
        switch (dayOfWeek) {
            case Calendar.SUNDAY: for(Vehicle v: vehicles) assertEquals(VehState.STOPPED, v.getState()); break;
            case Calendar.MONDAY: for (Vehicle v: vehicles) {
                if(v.getCurrNr() % 2 == 0)
                    assertEquals(VehState.MOVING, v.getState());
                else
                    assertEquals(VehState.STOPPED, v.getState());
            }  break;
            default: for(Vehicle v: vehicles) assertEquals(VehState.MOVING, v.getState()); break;
        }
    }
